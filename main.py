#@title 14 add exit date in log tab
...
# ── 2. 建立 Data Fetching Agent (適應最新版 yfinance) ──
def secured_download_agent(tickers, period="1y"):
...
# 執行 Agent 下載個股資料
closes, highs, lows, vols = secured_download_agent(WATCHLIST, period="1y")
...
for ticker in closes.columns:
    ...
    # ── 若技術面觸發，抓取基本面財報資料 (Lazy Fetching) ──
    if trigger_type:
        ...
        try:
            ...
            if raw_div > 1:
                div_yield_pct = round(raw_div, 2)
            else:
                div_yield_pct = round(raw_div * 100, 2)

            # 2. 解析業績反轉/成長動能
            earn_growth = tk_info.get('earningsGrowth') or tk_info.get('revenueGrowth') or 0
            earn_growth_pct = round(earn_growth * 100, 2)

            if earn_growth_pct >= 15:
                earn_label = f"強勁成長 (+{earn_growth_pct}%)"
            elif earn_growth_pct > 0:
                earn_label = f"溫和復甦 (+{earn_growth_pct}%)"
            elif earn_growth_pct < 0:
                earn_label = f"衰退中 ({earn_growth_pct}%)"
            else:
                earn_label = "未公佈"
...
for item in tracking_history:
    ...
    if 'exit_date' not in item: item['exit_date'] = "-" # 新增：結算日期欄位
    ...
    if item['status'] == 'Active' and item['ticker'] in closes.columns:
        ...
        if not pd.isna(latest_p):
            ...
            # 判斷是否觸發止盈/止損，並記錄發生的日期 (exit_date)
...
<head>
    ...
    <title>HK Quant Master V5.4 - 每日一篇教66歲丫媽學投資</title>
    <style>
...
<body class="p-6">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="card p-6 mb-6 flex flex-col md:flex-row justify-between items-center shadow-lg border-b-4 border-blue-500">
            <div>
                <h1 class="text-3xl font-black text-white mb-2">HK Quant Master V5.4 <span class="text-blue-500">技財雙重掃描器</span></h1>
                <p class="text-slate-400 text-sm">技術面發掘買點，基本面鎖定底氣 | 最後更新：{dashboard_data['date']}</p>
            </div>
        ...
        <div id="tab-scan" class="tab-content block">
            <div class="flex flex-col lg:flex-row gap-6">
                <div class="lg:w-2/5 flex flex-col gap-4 overflow-y-auto max-h-[850px] pr-2">
                    <h2 class="text-xl font-bold border-b border-slate-700 pb-2 sticky top-0 bg-[#0f172a] z-10 pt-2">🎯 今日觸發標的 ({len(signals)})</h2>
"""
...
html_content += f"""
                ...
                <div class="lg:w-3/5 flex flex-col">
                    ...
                    <div class="mt-4 card p-5 text-sm bg-slate-800">
                        <h3 class="font-bold text-blue-400 mb-3 border-b border-slate-700 pb-2 text-lg">💎 基本面長抱護城河評估</h3>
                        <div class="space-y-3">
                            <p class="text-slate-300">
                                <span class="bg-green-900 text-green-400 px-2 py-1 rounded text-xs mr-2 font-bold">高股息防護</span>
                                若該股<b>殖利率 > 6%</b>（綠色高亮），代表每年豐厚的配息能提供下行保護墊，適合長線佈局。
                            </p>
                            <p class="text-slate-300">
                                <span class="bg-blue-900 text-blue-400 px-2 py-1 rounded text-xs mr-2 font-bold">業績與估值</span>
                                若出現<b>「強勁成長」</b>且 <b>P/E偏低、ROE高</b>，這就是「被錯殺」的黃金坑。若業績為「衰退中」，抄底請務必嚴格執行止損！
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <div id="tab-manual" class="tab-content hidden card p-8 leading-relaxed">
            ...
            <h3 class="text-lg font-bold text-white mt-6 mb-2">2. 雙重交易引擎</h3>
            <ul class="list-disc pl-6 text-slate-300 mb-4 space-y-2">
                <li><strong class="text-green-400">牛市啟動：海龜突破策略 (順勢)</strong><br>
                當市場大好時，不猜頂，跟隨趨勢。系統會尋找突破過去 20 日最高價的股票，這代表資金正在湧入，適合順勢進場搭轎。</li>
                <li><strong class="text-red-400">熊市啟動：RSI 抄底策略 (逆勢)</strong><br>
                當市場恐慌時，尋找被錯殺的股票。系統會找出同時跌破「布林通道下軌」且「RSI低於30(極度超賣)」的股票，這些股票通常會有技術性反彈的需求。</li>
            </ul>
...
print(f"🎉 成功！已加入止盈止損日期紀錄功能，生成完美 V5.4 儀表板：{filename}")
try:
